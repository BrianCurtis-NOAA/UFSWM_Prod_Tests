...
# Bring items from build & ufswm into our run storage area
mv "${BUILD_DIR}/ufs_model" "${PATHRT}/run/${BUILD_NAME}.exe"
if [[ ${MACHINE_ID} == linux ]]; then
  cp "${PATHTR}/modulefiles/ufs_${MACHINE_ID}.${RT_COMPILER}" "${PATHRT}/run/modules.${BUILD_NAME}"
else
  cp "${PATHTR}/modulefiles/ufs_${MACHINE_ID}.${RT_COMPILER}.lua" "${PATHRT}/run/modules.${BUILD_NAME}.lua"
fi
