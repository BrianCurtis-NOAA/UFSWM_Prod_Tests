...
write_fail_test() {
  echo "${TEST_ID} failed in run_test" >> "${PATHRT}/run/fail_test_${TEST_ID}"
  exit 1
}

remove_fail_test() {
    echo "Removing test failure flag file for ${TEST_ID}"
    rm -f "${PATHRT}/run/fail_test_${TEST_ID}"
}
...
# FV3 executable:
cp "${PATHRT}/run/fv3_${COMPILE_ID}.exe" "fv3.exe"

# modulefile for FV3 prerequisites:
mkdir -p modulefiles
if [[ ${MACHINE_ID} == linux ]]; then
  cp "${PATHRT}/run/modules.fv3_${COMPILE_ID}" "./modulefiles/modules.fv3"
else
  cp "${PATHRT}/run/modules.fv3_${COMPILE_ID}.lua" "./modulefiles/modules.fv3.lua"
fi
...
if [[ ${delete_rundir} = true ]]; then
  ...
  done < "${PATHRT}/run/keep_tests.tmp"
