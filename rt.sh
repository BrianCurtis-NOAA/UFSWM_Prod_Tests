...
PATHTR=$( cd "${PATHRT}/ufs-weather-model" && pwd )
readonly PATHTR
...
if mkdir "run" ; then
  ...
  if mv "run" "run_old" ; then
    if mkdir "run" ; then
      echo "Moved run/ to run_old/ and created new run/ directory"
    else
      echo "Error creating new run/ directory.. Exiting..."
      exit 1
    fi
  else
    echo "Error moving run/ to run_old/.. Exiting..."
    exit 1
  fi
fi

# make sure only one instance of rt.sh is running
readonly LOCKDIR="${PATHRT}"/run/lock
...
echo "Linking ${RUNDIR_ROOT} to ${PATHRT}/run/run_dir"
ln -s "${RUNDIR_ROOT}" "${PATHRT}/run/run_dir"
...
if [[ ${ROCOTO} == true ]]; then
  ...
  ROCOTO_XML=${PATHRT}/run/rocoto_workflow.xml
  ROCOTO_STATE=${PATHRT}/run/rocoto_workflow.state
  ROCOTO_DB=${PATHRT}/run/rocoto_workflow.db
...
if [[ ${ECFLOW} == true ]]; then
  ...
  ECFLOW_RUN=${PATHRT}/run/ecflow_run
